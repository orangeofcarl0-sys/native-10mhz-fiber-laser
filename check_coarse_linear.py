"""Same-state full-model Fourier-subspace preconditioner measurement."""
import os,json,time
...
for cutoff in [32,64]:
    ...
    row=dict(cutoff=cutoff,info=int(info),relative_linear_residual=float(error),
             calls=residual.evaluations-calls,construction_calls=construction_calls,
             seconds=time.perf_counter()-start,**{k:v for k,v in diagnostics.items() if k!='cutoff'})
    rows.append(row)
    np.savez_compressed(ROOT/f'linear_coarse_{cutoff}.npz',x=x,dx=dx)
    (ROOT/'linear_coarse.json').write_text(json.dumps(rows,indent=2),encoding='utf8')
