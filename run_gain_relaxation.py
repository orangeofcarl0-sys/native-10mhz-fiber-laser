...
def periods(fields, pops, qs):
    ...
    for p in range(1, 17):
        errors = np.array(
            [field_distance(fields[k - p], fields[k]) for k in range(p, len(fields))]
        )
        nr = max(
            np.linalg.norm(pops[k] - pops[k - p]) / np.linalg.norm(pops[k - p])
            for k in range(p, len(pops))
        )
        na = max(float(np.max(abs(pops[k] - pops[k - p]))) for k in range(p, len(pops)))
        row = dict(
            period=p,
            intensity=float(errors[:, 0].max()),
            field=float(errors[:, 1].max()),
            stokes=float(errors[:, 2].max()),
            population=float(nr),
            population_abs=na,
...
def grid_probe(c, a, pop, q, dt, rounds=8):
    ...
    """
    reference = BatchEngine(c, dt, a.shape[-1], [0.05], [0.8])
    fine_c = dict(
        c, max_step_m=c["max_step_m"] / 2, passive_step_m=c["passive_step_m"] / 2
    )
    b, fdt = regrid(a.copy(), dt, finer=True)
    cells = int(np.ceil(c["segments"][1, 0] / fine_c["max_step_m"]))
    pb = remap_population(pop, cells)
    qb = q.copy()
    fine = BatchEngine(fine_c, fdt, b.shape[-1], [0.05], [0.8])
    ...
    result["below_screen_tolerances"] = all(
        result[k] <= LIMITS[k] for k in result if k in LIMITS
    )
    return result
...
def run():
    ...
    c = config("CNT_OC", 0.2)
    hashes = {
        n: hashlib.sha256((PROJECT / n).read_bytes()).hexdigest()
        for n in [
            "spectral_engine.py",
            "gpu_kernels.py",
            "gpu_engine.py",
            "gain_age.py",
            "run_gain_relaxation.py",
    ...
    if checkpoint.exists():
        ...
        if manifest["status"] != "running":
            print("Run already finalized", flush=True)
            return
        if manifest["code_sha256"] != hashes:
            raise RuntimeError("Resume requires unchanged code")
        with np.load(checkpoint) as m:
            a, pop, q, age = (m[k].copy() for k in ["a", "pop", "q", "age"])
            dt = float(m["dt"])
            done = int(m["done"])
        trace = json.loads((ROOT / "trace.json").read_text())[:done]
        if len(trace) != done or manifest["accepted_additional_rounds"] != done:
            raise RuntimeError(
                "Checkpoint and trace were not saved consistently; do not silently resume"
            )
    else:
        m = loadmat(source, variable_names=["a", "pop", "q", "completed"])
        assert m["completed"].ravel()[53] == 600
        a = m["a"][53:54].copy()
        pop = m["pop"][53:54].copy()
        q = m["q"].ravel()[53:54].copy()
        dt = 0.125
        age = np.zeros_like(pop)
        done = 0
        trace = []
        manifest = dict(
            status="running",
            ...
            primary_budget=10000,
            maximum_budget=20000,
            gain_age_gate=7,
            recurrence_screen_limits=LIMITS,
            events=[],
            period_checks=[],
            started_utc=__import__("datetime")
            .datetime.now(__import__("datetime").timezone.utc)
            .isoformat(),
        )
        # Check the new fused B diagnostic independently against the CPU engine.
        eng = BatchEngine(c, dt, a.shape[-1], [0.05], [0.8])
        cpu = CPUReference(c, dt, a.shape[-1], [0.05], [0.8])
        ga = eng.step(a.copy(), pop.copy(), q.copy())
        ca = cpu.step(a.copy(), pop.copy(), q.copy())
        np.testing.assert_allclose(eng.rates_b, cpu.rates_b, rtol=1e-11)
        for actual, expected in zip(ga, ca):
            np.testing.assert_allclose(actual, expected, rtol=1e-9, atol=1e-10)
        manifest["gpu_rate_validation_max_relative"] = float(
            np.max(abs(eng.rates_b / cpu.rates_b - 1))
        )
        manifest["initial_grid_probe"] = grid_probe(c, a, pop, q, dt)
        print("INITIAL VALIDATION", manifest["initial_grid_probe"], flush=True)
    engine = BatchEngine(c, dt, a.shape[-1], [0.05], [0.8])
    ...
    def save():
        ...
        manifest.update(
            status=status,
            accepted_additional_rounds=done,
            total_rounds=600 + done,
            **summary(age)
        )
    ...
    while done < 20000:
        ...
        if edges[0] > 1e-6 or edges[1] > 1e-8:
            if a.shape[-1] * 2 > 65536 or retries >= 8:
                ...
                manifest["failed_edges"] = edges.tolist()
                break
            wider = bool(edges[0] > 1e-6)
            finer = not wider
            a, dt = regrid(a, dt, finer=finer, wider=wider)
            manifest["events"].append(
                dict(
                    at_total_round=600 + done,
                    action="rollback_widen" if wider else "rollback_finer_dt",
                    dt_ps=dt,
                    n=a.shape[-1],
                )
            )
            ...
            qs.clear()
            retries += 1
            print("RETRY", manifest["events"][-1], flush=True)
            continue
        retries = 0
        ...
        values = dict(
            round=600 + done,
            energy_pJ=float(power.sum() * dt),
            cnt_peak_W=float(sa[0, 1]),
            cnt_q_min=float(sa[0, 2]),
            inversion_gap=float(engine.last_gap[0]),
            tau_max_us=float(engine.last_tau[0] * 1e6),
            peaks=peak_count,
        ...
        trace.append(values)
        fields.append(a[0].copy())
        pops.append(pop[0].copy())
        qs.append(float(q[0]))
        if done % 500 == 0:
            ...
            print(
                dict(
                    round=600 + done,
                    age=values["gain_age_min"],
                    energy_nJ=values["energy_pJ"] / 1000,
                    cv500=float(tail.std() / max(tail.mean(), 1e-250)),
                    peaks=peak_count,
                    best_field=min((v["field"] for v in pr), default=None),
        ...
        if done >= 10000 and summary(age)["gain_age_min"] >= 7:
            status = "time_budget_complete_gain_age_reached"
            break
    if status == "running":
        status = "maximum_budget_reached"
    manifest["final_periods"] = periods(fields, pops, qs)
    manifest["final_grid_probe"] = (
        grid_probe(c, a, pop, q, dt)
        if a.shape[-1] <= 32768
        else {"status": "resource_limit"}
    )
    ...
    if passed:
        manifest["classification"] = (
            "finite_record_recurrence_candidate_needs_long_hold_and_perturbation_tests"
        )
        manifest["candidate_period"] = passed[0]["period"]
    if status == "numerical_boundary_unresolved":
    ...
    save()
    print(
        "FINISHED", status, done, summary(age), manifest["classification"], flush=True
    )
