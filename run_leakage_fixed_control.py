...
rfun=parameter_residual(state,'pump',float(last['y'][-1]),gpu=True)
x=last['x'];r=rfun(x)
...
for mode,threshold in [('baseline',10),('truncate',5),('truncate',10),('truncate',20),('cap',10)]:
    ...
    (ROOT/'leakage_fixed_control.json').write_text(json.dumps(dict(rows=rows,seconds=time.perf_counter()-start),indent=2))
np.savez_compressed(ROOT/'leakage_fixed_modes.npz',singular=data[3],right=data[4][-20:],left=data[2][:,-20:])
