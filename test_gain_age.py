...
class GainAgeTests(unittest.TestCase):
    def test_continuation_stops_without_an_anchor(self):
        ...
        def unresolved(*args, **kwargs):
            return (
                {"status": "nonstationary_or_unresolved"},
                (np.zeros((1, 2, 8)), np.zeros((1, 1)), np.zeros(1)),
                0.5,
            )

        with TemporaryDirectory() as directory, patch.object(
            attractor_search, "ROOT", Path(directory)
        ), patch.object(
            attractor_search, "run_point", side_effect=unresolved
        ) as runner:
            rows = attractor_search.continuation("CNT_OC", 0.2, 0.8, [30, 40, 50])
            self.assertEqual(runner.call_count, 2)
            self.assertTrue(
                all(r["branch_stopped_without_converged_anchor"] for r in rows)
            )
    ...
    def test_frozen_rate_conditional_memory(self):
        ...
        for _ in range(70):
            age = advance(age, rates, 0.05)
            delta *= np.exp(-rates * 0.05)
        np.testing.assert_allclose(np.exp(-age), delta, rtol=1e-13)
        self.assertAlmostEqual(summary(age)["gain_age_min"], 7.0)

    def test_cellwise_sum_before_minimum_and_resume(self):
        age = advance(np.zeros(2), [1.0, 10.0], 1.0)
        resumed = advance(age.copy(), [10.0, 1.0], 1.0)
        np.testing.assert_array_equal(resumed, [11.0, 11.0])
        self.assertEqual(summary(resumed)["gain_age_min"], 11.0)
        with self.assertRaises(ValueError):
            advance(age, [1.0], 1.0)

    def test_rates_match_reference_without_changing_physics(self):
        c = config("CNT_OC", 0.2)
        a, p, q = map_initial(c, [0.8], n=512)
        reference = BatchEngine(c, 0.5, 512, [0.05], [0.8])
        spectral = SpectralEngine(c, 0.5, 512, [0.05], [0.8])
